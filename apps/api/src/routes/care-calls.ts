...
import {
  CareCallSession,
  pendingFromPlan,
  type PendingCareCall,
} from '../services/care-call/runtime.js';
...
export async function careCallRoutes(app: FastifyInstance) {
  app.get('/care-calls/twilio/stream', { websocket: true }, (socket) => {
    ...
    let activeAgentMouth: ReturnType<typeof openMouth> | null = null;
    let session: CareCallSession | null = null;
    ...
    function stopSpeech() {
      speaking?.abort();
      speaking = null;
      activeAgentMouth?.close();
      activeAgentMouth = null;
      clearCallAudio();
    }
    ...
    function agentSpeech(ctrl: AbortController) {
      const speech = new AbortController();
      stopSpeech();
      speaking = speech;
      const mouth = openMouth({
      ...
      return {
        ...
        close: () => {
          mouth.close();
          if (speaking === speech) speaking = null;
          if (activeAgentMouth === mouth) activeAgentMouth = null;
        },
      };
    }
    ...
    const ear = openEar(
      {
        ...
        onFinal: (text) => session?.hearFinal(text),
    ...
    socket.on('message', (data: Buffer) => {
      ...
      if (msg.event === 'start') {
        ...
        const call = pendingCareCalls.get(params.sessionId ?? '') ?? {
        ...
        session = new CareCallSession(call, 'care-call', {
          speak: speakToCall,
          agentSpeech,
          closeAfter,
        ...
        }, `twilio_stream_${streamSid ?? randomUUID()}`);
        session.open();
        return;
      }
    ...
    socket.on('message', (data: Buffer, isBinary: boolean) => {
      ...
      try {
        ...
        if (msg.type === 'start' && msg.sessionId) {
          const call = pendingCareCalls.get(msg.sessionId) ?? null;
          ...
          session = new CareCallSession(call, 'care-call-test', {
            speak: speakToBrowser,
            agentSpeech,
            closeAfter,
            interruptSpeech: () => stopSpeech(true),
            onDesk: (desk: Desk) => {
              activeAgentMouth?.setSpeaker(voiceFor(desk));
              send({ type: 'desk', desk });
            },
            onTurn: (turn) => send({ type: 'turn', ...turn }),
            onLog: (event, payload) => app.log.info(payload, event),
            onError: (message) => send({ type: 'error', message }),
          }, 'care_test');
          session.open();
          return;
        }
        if (msg.type === 'text' && typeof msg.text === 'string') session?.hearFinal(msg.text);
        if (msg.type === 'stop') {
          session?.close();
        }
      } catch {
